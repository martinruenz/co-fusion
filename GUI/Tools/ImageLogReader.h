...
class ImageLogReader : public LogReader {
 public:
  ...
  // (Mask0001.pgm/png)
  FrameData loadFrameFromDrive(const size_t& index);
 ...
 private:
  ...
  size_t maxMasks = 0;  // Max number of masks that should be used (in order to initialise with ground truth)
