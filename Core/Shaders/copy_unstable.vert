...
out vec4 vColor;
out vec4 vNormRad;
flat out int test; // =1: point gets copied, =0: point does not get copied

uniform int time;
uniform float scale;
uniform float outlierCoeff;
uniform mat4 t_inv;
...
uniform sampler2D depthSamplerPrediction;
uniform sampler2D depthSamplerInput;
uniform float nodes;
...
void main()
{
    ...
    float indexXStep = stepX * 0.5f / scale; //TODO Why *0.5 ?
    float indexYStep = stepY * 0.5f / scale;
    ...
    // Is this meant to supress dublicates?
    if(//abs(localNorm.z) > 0.85f &&
       time - vColor.w < timeDelta && localPos.z > 0 && x > 0 && y > 0 && x < cols && y < rows)
    {
        for(float i = x_n - (scale * indexXStep * windowMultiplier); i < x_n + (scale * indexXStep * windowMultiplier); i += indexXStep){
        ...
        for(float i = x_n - stepX; i <= x_n + stepX; i += stepX){
            for(float j = y_n - stepY; j <= y_n + stepY; j += stepY){
                float d = float(textureLod(depthSamplerInput, vec2(i, j), 0)) - localPos.z;
                if(d > 0.03) {
                  violationCount++;
                  avgViolation += d;
                }
    ...
    if(violationCount > 0) {
      ...
      //TODO remove
      //if(maskValue != maskID) vPosition.w *= (0.5 + 0.5 * outlierCoeff);
    }


    // Loop-closing (applying deformation?):
    ...
    if(test == 1 && nodes > 0 && vColor.z != time)
    {
        ...
        if(abs(nodeMin - poseTime) <= abs(nodeMid - poseTime) && abs(nodeMin - poseTime) <= abs(nodeMax - poseTime))
            foundIndex = imin;
        else if(abs(nodeMid - poseTime) <= abs(nodeMin - poseTime) && abs(nodeMid - poseTime) <= abs(nodeMax - poseTime))
            foundIndex = imid;
        else
            foundIndex = imax;
