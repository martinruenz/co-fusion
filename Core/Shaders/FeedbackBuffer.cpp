...
void FeedbackBuffer::compute(pangolin::GlTexture* color, pangolin::GlTexture* depth, const int& time, const float depthCutoff) {
  program->Bind();

  Eigen::Vector4f cam(Intrinsics::getInstance().cx(), Intrinsics::getInstance().cy(), 1.0f / Intrinsics::getInstance().fx(),
  ...
  glDrawArrays(GL_POINTS, 0, Resolution::getInstance().numPixels());  // GPU-PASS
...
void FeedbackBuffer::render(pangolin::OpenGlMatrix mvp, const Eigen::Matrix4f& pose, const bool drawNormals, const bool drawColors) {
  drawProgram->Bind();
  ...
  glDrawTransformFeedback(GL_POINTS, fid);  // RUN GPU-PASS
