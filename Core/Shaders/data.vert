...
out vec4 vPosition; // 3d position + confidence
out vec4 vColor; // w-component:    -1: found point to merge, -2: new unstable
out vec4 vNormRad;
flat out int updateId; //            1: found point to merge,  2: new unstable
...
void main()
{
    ...
    updateId = 0; // 1: update, 2: new unstable, 3: penalise

    uint best = 0U;

    uint maskValue = uint(textureLod(maskSampler, texcoord.xy, 0.0));
    ...
    {
        //bool merge = false;
        int operation = 0; // 0 => new point, 1 => merge, ~~3 => penalise~~
            ...
            for(float j = texcoord.y - (scale * indexYStep * windowMultiplier); j < texcoord.y + (scale * indexYStep * windowMultiplier); j += indexYStep){
                ...
                if(current > 0U){
                    ...
                    if(abs(zdiff * lambda) < 0.05){
                        ...
                        // Check normal conformance, radius size and whether it is the best distance
                        ...
                        //else if(/*operation != 1 &&*/ zdiff < 0.01 /*|| ICP*/) { //FIXME ICP
                    //    // Penalise confidence, if depth data is "looking through" object
                    //    operation = 3;
                    //}
                }
            }
        ...
        //TODO Remove
        // Outlier rejection (Martin)
        //uint current = uint(textureLod(indexSampler, vec2(texcoord.x, texcoord.y), 0.0));
        //if(current > 0U){
        //    vec4 vertConf = textureLod(vertConfSampler, vec2(texcoord.x, texcoord.y), 0.0);
        //    float zdiff = (vertConf.z - vPosLocal.z);
        //    if(/*operation != 1 &&*/ zdiff > 0.1 /*|| ICP*/) {
        //        operation = 3;
        //    }
        //}
