...
void DeformationGraph::applyGraphToPoses(std::vector<Eigen::Matrix4f*>& poses) {
  ...
  for (size_t i = 0; i < poses.size(); i++) {
    ...
    for (size_t j = 0; j < weightMap.size(); j++) {
      newPosition += weightMap.at(j).weight * (graph.at(weightMap.at(j).node)->rotation *
...
void DeformationGraph::setPosesSeq(std::vector<unsigned long long int>* poseTimeMap, const std::vector<Eigen::Matrix4f>& poses) {
  ...
  for (unsigned int i = 0; i < poses.size(); i++) {
    ...
    if (std::llabs(sampledGraphTimes[imin] - poseTime) <= std::llabs(sampledGraphTimes[imid] - poseTime) &&
        std::llabs(sampledGraphTimes[imin] - poseTime) <= std::llabs(sampledGraphTimes[imax] - poseTime)) {
      foundIndex = imin;
    } else if (std::llabs(sampledGraphTimes[imid] - poseTime) <= std::llabs(sampledGraphTimes[imin] - poseTime) &&
               std::llabs(sampledGraphTimes[imid] - poseTime) <= std::llabs(sampledGraphTimes[imax] - poseTime)) {
      foundIndex = imid;
    } else {
      foundIndex = imax;
    }
    ...
    std::sort(nearNodes.begin(), nearNodes.end(),
    ...
    for (unsigned int j = 0; j < (unsigned int)k; j++) {
      newMap.push_back(VertexWeightMap(pow(1.0f - (vertexPosition - graphNodes[nearNodes.at(j).second].position).norm() / dMax, 2),
...
void DeformationGraph::weightVerticesSeq(std::vector<unsigned long long int>* vertexTimeMap) {
  ...
  for (unsigned int i = lastPointCount; i < sourceVertices->size(); i++) {
    ...
    if (std::llabs(sampledGraphTimes[imin] - vertexTime) <= std::llabs(sampledGraphTimes[imid] - vertexTime) &&
        std::llabs(sampledGraphTimes[imin] - vertexTime) <= std::llabs(sampledGraphTimes[imax] - vertexTime)) {
      foundIndex = imin;
    } else if (std::llabs(sampledGraphTimes[imid] - vertexTime) <= std::llabs(sampledGraphTimes[imin] - vertexTime) &&
               std::llabs(sampledGraphTimes[imid] - vertexTime) <= std::llabs(sampledGraphTimes[imax] - vertexTime)) {
      foundIndex = imid;
    } else {
      foundIndex = imax;
    }
...
void DeformationGraph::clearConstraints() { constraints.clear(); }

bool DeformationGraph::optimiseGraphSparse(float& error, float& meanConsErr, const bool fernMatch,
    ...
    for (unsigned int n = 0; n < graph.at(j)->neighbours.size(); n++) {
      if (graph.at(graph.at(j)->neighbours.at(n))->enabled || graph.at(j)->enabled) {
        residual.segment(numRows, 3) =
            (graph.at(j)->rotation * (graph.at(graph.at(j)->neighbours.at(n))->position - graph.at(j)->position) + graph.at(j)->position +
             graph.at(j)->translation -
             (graph.at(graph.at(j)->neighbours.at(n))->position + graph.at(graph.at(j)->neighbours.at(n))->translation))
                .cast<double>() *
            sqrt(wReg);
        numRows += eRegRows;
      }
...
void DeformationGraph::computeVertexPosition(int vertexId, Eigen::Vector3f& position) {
  ...
  for (unsigned int i = 0; i < weightMap.size(); i++) {
    position +=
        weightMap.at(i).weight * (graph.at(weightMap.at(i).node)->rotation * (sourcePosition - graph.at(weightMap.at(i).node)->position) +
